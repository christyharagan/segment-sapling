...
import { LIB_DIR, load_project_settings, TPS_DIR } from './project_settings'
import { TrackingPlanCreation } from 'segment-typescript-api/cjs/config_request'


export async function publish(_access_token?: string, _work_slug?: string) {
  ...
  await Promise.all(fs.readdirSync(TPS_DIR).map(async f => {
    ...
    if (fs.lstatSync(full_f).isDirectory()) {
      ...
      if (links) {
        const libs = await Promise.all(links.map(async link => {
          ...
          if (fs.lstatSync(full_f).isDirectory()) {
            ...
            throw 'links setting in settings.json is invalid. Cannot find library folder: ' + full_f
          }
        }))
      ...
      const existing_tp = segment_tps.tracking_plans.find(tp => tp.display_name == f)
      if (existing_tp) {
